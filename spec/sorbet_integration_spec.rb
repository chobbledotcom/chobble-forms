...
RSpec.describe "Sorbet Integration" do
  it "extends modules with T::Sig" do
    expect(ChobbleForms).to respond_to(:sig)
    expect(ChobbleForms::Helpers).to respond_to(:sig)
  end
  ...
  it "allows helper methods to work with Sorbet" do
  ...
  it "has properly typed VERSION constant" do
    expect(ChobbleForms::VERSION).to be_a(String)
    expect(ChobbleForms::VERSION).to match(/^\d+\.\d+\.\d+/)
  end
end